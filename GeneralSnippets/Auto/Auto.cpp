...
namespace Auto_Examples {
    ...
    static void test_01() {
        ...
        auto   z1 = 123.456;  // Type Deduction // Typableitung

        double z2 = 123.456;  // Type Deduction // Typableitung
    ...
    static void test_03() {
        ...
        // std::pair<const int, std::string>& entry3 = *it;
    ...
    static decltype ( std::declval<T>() + std::declval<U>() )
        tueWasSeminar(bool flag, T f, U d)
    {
        if (flag) {
            return d;
        }
    ...
    void test_tue_was()
    {
        auto result = tueWasSeminar<>(true, 123l, 123);
    }
