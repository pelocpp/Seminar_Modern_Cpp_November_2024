...
namespace VariadicTemplatesIntro_Seminar {
	...
	template <typename T, typename ... TRest> //         einpacken (Datentypen)
	void printer(T n, TRest ... args) {          // pack // einpacken (Wert)
		std::cout << n << std::endl;
		printer (args ...);
	}
	...
	void test_seminar_01() {
		...
		auto result = addiererModern(1, 2, 3, 4, 5);
	...
	void test_seminar() {
		...
		std::unique_ptr<Unknown> up 
			= my_make_unique<Unknown>(1.5, 2.5);
	}
}
